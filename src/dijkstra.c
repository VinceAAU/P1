 ...
 * @param current_time
 * @param first_station
 * @param second_station
 * @return the time it takes to switch train when going from first_station to second_station
 */
 ...
 * @param first_station
 * @param second_station
 * @return the lineID of the route of the connection between two stations
 */
...
{
    int cost[number_of_stations][number_of_stations],
    distance[number_of_stations],
    predecessor[number_of_stations],
    visited[number_of_stations],
    count,
    min_distance,
    next_node,
    i, j,
    ...
    // sets all the zeros to infinity
    // this could be done in create_adjacency_matrix(), but is done here so the matrix is readable when printed using print_matrix()
    ...
    for (i = 0; i < number_of_stations; i++) {
        distance[i] = cost[startnode][i]; // set distances to nodes that connect to start node
        predecessor[i] = startnode;  // set path to only include the start node, since that's the only known one
        visited[i] = 0; // set all nodes to unvisited
    }
    ...
    while (count < number_of_stations - 1) {
        ...
        //checks if a smaller path to next exists, and adds it to the path if it does
        for (i = 0; i < number_of_stations; i++)
            if (!visited[i])
    ...
    // find out how long the optimal_path is
    j = endnode;
    int counter = 1;
    do {
        j = predecessor[j];
    ...
    // fill the optimal_path array with the correct stations, which the algorithm has found.
    j = endnode;
    ...
    // reverses array since the algorithm find the stations in reversed order
    ...
    // starts the time when the first train/plane arrives at the start station

    for(i = 0; i < station_list_length(optimal_path); i++ )
    {
        if(i >= 1) {
            current_time += distance[optimal_path[i].id-65]-distance[optimal_path[i-1].id-65];
            //adds the time the train/plane takes to travel to the current_time for all connections except the first.
            // since distance[] is the total time it takes to travel to a node from the startnode, the distance to the last station is subtracted to get the specific time for this connection.
        }
        else {
            current_time += distance[optimal_path[i].id-65]; // adds the time the train/plane takes to travel to the current_time for the first connection
        }
        if(current_line == 0) {
            current_line = check_line_switch(optimal_path[i],optimal_path[i+1]); // sets the first line
        }
        if(check_line_switch(optimal_path[i],optimal_path[i+1]) != current_line){
            ...
            if(current_time >= MIDNIGHT) {
                current_time = current_time - MIDNIGHT; // if the time passes midnight, start a new day
            }
            current_time += get_route_switch_time(current_time,optimal_path[i],optimal_path[i+1]);
            if(current_time < start_time) {
                total_time = current_time+ MIDNIGHT; // add the day back to total time, if it was subtracted earlier
            } else {
